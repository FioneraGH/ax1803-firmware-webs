...
function hideEditNameArea(rowEle, devName) {
	$(rowEle).find(".dev-name").html(toHtmlCode(devName)).attr("title", devName).append('<img class="edit-btn edit-btn-txt-append" src="img/edit_new.png" style="width:14px;height:14px;"/>');
}
...
function updateData(dataList) {
	...
	initDataList.sort((function () {
		...
		return function (item1, item2) {
			var a = item1.offline.match(splitter),
...
function drawList(dataList) {
	...
	$("#netBody tr").each(function () {
		...
		for (var i = dataList.length - 1; i >= 0; i--) {
			...
			if (mac == rowData.mac) {
                ...
                hostName = toHtmlCode(rowData.hostName || top.G.deviceNameSpace);

				$(this).find('[alt=hostName]').html(hostName);
	...
	for (i = 0; i < dataList.length; i++) {
		if (!dataList[i].exist) {
			var data = dataList[i],
				elem = $("[alt='" + dataList[i].mac + "']");
			limitUp = (!data.upSpeed && !data.downSpeed) ? data.limitUp : ((parseFloat(data.limitUp) / 128).toFixed(2));
			limitDown = (!data.upSpeed && !data.downSpeed) ? data.limitDown : ((parseFloat(data.limitDown) / 128).toFixed(2));
			elem.find(".dev-name").attr("title", data.hostName || top.G.deviceNameSpace);
			elem.find("span[alt=limitUp]").toSelect(selectObjUp)[0].val(limitUp);
			elem.find("span[alt=limitDown]").toSelect(selectObjDown)[0].val(limitDown);
			elem.find("[alt=limitDown] .dropdown-menu").css("left", $("#netBody tr:last").find("span[alt=limitDown]").width() - $("#netBody tr:last").find("[alt=limitDown] .dropdown-menu").width() + "px");
			elem.find("input[type=text]").inputCorrect("float").on("focus", function () {
...
function addRow(obj) {
	...
	str += "<td class='edit-td'>" + 
				"<div class='device-icon'>" + 
					"<img src='" + deviceType.src + "'>" + 
					showDeviceLogoString(deviceType, obj.devType) + 
				"</div>" + 
				"<div class='online-device-content'>" + 
					"<div class='dev-name text-fixed' style='padding-right: 30px;'>" + 
						"<span class='dev-name-txt'>" + toHtmlCode(obj.hostName || top.G.deviceNameSpace) +
						"</span>" + 
						"<img class='edit-btn edit-btn-txt-append' src='img/edit_new.png' style='width:14px;height:14px;' />" + 
					"</div>" + 
					"<div class='txt-help-tips'>" + (obj.ip || "---") + 
					"</div>" + 
				"</div>" + 
			"</td>";
