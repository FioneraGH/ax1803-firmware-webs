	...
	G.validate = $.validate({
		custom: function () {
			var guestName = $('#guestuser').val(),
                internetPort = $("#internetPort").val(),
                vsport = initObj.vsport.split("-"),
                portRepeat = false,
                returnObj = {},
                i = 0;
        
            for (; i < vsport.length; i++) {
                if (internetPort == vsport[i]) {
                    portRepeat = true;
                    break;
                }
            }
            ...
            if (portRepeat) {
                returnObj = {
                    "id": "msg-err-ftp",
                    "msg": _("The port for internet access cannot be duplicate with the external port of virtual server")
                };
                return  returnObj;
            }
			...
			if ($("#upnpBody tr").length === 2) {
				if (guestName === "admin") {
                    returnObj = {
                        "id": "guest-user-err",
                        "msg": _("The user name of the guest cannot be admin.")
                    };
					return returnObj;
				}
			}
