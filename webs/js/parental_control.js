...
function checkParentData() {
    ...
    if ($("#parentcontrolEnable").val() == "1") {
        ...
        if ($("#whiteEnable").val() == "1") {
            ...
            if (len > 32) {
                if ($('[name="limitType"]:checked').val() === "1") {
                    return _("Only a maximum of %s whitelisted URLs are allowed.", [32]);
                } else {
                    return _("Only a maximum of %s blacklisted URLs are allowed.", [32]);
                }
                return;
            }
            result = [];
        ...
        if (G_current_operate === "1") {
            ...
            if (!$("#device_save").hasClass("none")) {
                ...
                msg = checkDevNameValidity(editDeviceName);
...
function changeLimitType() {
    if ($("#black")[0].checked) {
        $("#limit_label").html(_("Blocked Websites:")+"<span>&nbsp;&nbsp;</span>");
        ...
        $("#parent_urls").val('');
        if(G.initObj && (G.initObj.limit_type == 0)){
            $("#parent_urls").val(G.initObj.urls);
        }
    } else {
        $("#limit_label").html(_("Unblocked Websites:")+"<span>&nbsp;&nbsp;</span>");
        ...
        $("#parent_urls").val('');
        if(G.initObj && (G.initObj.limit_type == 1)){
            $("#parent_urls").val(G.initObj.urls);
        }
    }
}
...
function editDevice(action) {
    ...
    if (action == "edit") {
        ...
        //统一验证设备名称合法性
        msg = checkDevNameValidity(deviceName);
