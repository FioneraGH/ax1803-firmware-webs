			...
			switch (tagName) {
				...
				default:
					if ($("#" + prop).hasClass("textboxs") || $("#" + prop).hasClass("input-append")) {
						$("#" + prop)[0].val(obj[prop]);
					} else if ($("#" + prop).hasClass("btn-on") || $("#" + prop).hasClass("btn-off")) {
						$("#" + prop).val(obj[prop]);
						if (obj[prop] == "1") {
							$("#" + prop).removeClass("btn-off").addClass("btn-on");
						} else {
							$("#" + prop).removeClass("btn-on").addClass("btn-off");
						}
					}
					break;
			}
...
/**
 * @method 对从后台获取到的数据进行转码 by pjl
 * @param  {String} str [待转换的数据]
 * @return {String}     [转换后的数据]
 */

var toHtmlCode = function (str) {
...
function showErrMsg (id, str, noFadeAway, time) {
	var T = 0;
	var showErrMsgFun = function (id, str, noFadeAway, time) {
		clearTimeout(T);
		$("#" + id).html(str);
		time = time || 2000;
		if (!noFadeAway) {
	...
	showErrMsgFun(id, str, noFadeAway, time);
}
...
function showSaveMsg (num, str, flag, change) {
	...
	if (num == 0) {
		...
		hideDialog = true; //不隐藏弹出框
		$(".save-msg").removeClass("none");
		$("#page-message").html(_("正在上传文件"));
		$("#page-message").removeClass("none");
		//top.location.reload(true);
	} else {
		$(".save-msg").removeClass("none");
		$("#page-message").html(_("Saving success"));
...
function rebooting (time, type) {
	...
	if (pc <= 100) {
		...
		rebootTime = setTimeout('rebooting(' + time + ',"' + type + '")', time);
		//$(".load_pc").css("width", pc+'%');
		$("#load_pc").attr("class", "up-loadding load-reboot");
		$("#load_text").html(pc + '%');

		//预估最快1分半钟可以升级完成 90/154 = 0.62
		//1分半钟后发送请求，若有数据返回则升级成功，加快进度条
		if (type === "upgrade" && pc > 50) {
			// console.log("开始获取数据");
			upgradeTime && clearTimeout(upgradeTime);
			upgradeTime = window.setTimeout(function () {
				$.get("goform/GetSysAutoRebbotCfg?" + Math.random(), function () {
					// console.log("数据已返回");
					upgradeTime && clearTimeout(upgradeTime);
					upgradeTime = null;
					rebootTime && clearTimeout(rebootTime);
					rebootTime = setTimeout('rebooting(' + 150 + ')', 150);
				});
			}, 0);
		}
		pc++;
		...
		clearTimeout(rebootTime);
		pc = 1;
		if (ipaddress != "" && (/^([1-9]|[1-9]\d|1\d\d|2[0-1]\d|22[0-3])\.(([0-9]|[1-9]\d|1\d\d|2[0-4]\d|25[0-5])\.){2}([0-9]|[1-9]\d|1\d\d|2[0-4]\d|25[0-5])$/).test(ipaddress) == true || ipaddress == top.G.domain) {
...
(function ($) {
	...
	var setPercent = function (ele, percent, showTxt) {
		showTxt = showTxt ? showTxt : percent + "%";
	...
	$.fn.toProgress = function (percent, width, showTxt) {
		percent = percent ? (parseFloat(percent) <= 100 && parseFloat(percent) >= 0 ? parseFloat(percent) : 0) : 0;
...
function translateSpeed (speedKB) {
	if (Number(speedKB) < 1024) {
		return Number(speedKB).toFixed(1) + _("KB/s");
	} else {
		return (speedKB / 1024).toFixed(1) + _("MB/s");
	}
}
...
R.extend(R, {
	...
	isTimeOut: function (str) {
		if (str.indexOf("<!DOCTYPE html") === 0 && str.indexOf("login.js") != -1) {
			return true;
		}
...
function checkDevNameValidity (devName, isCanNull) {
	...
	//edit by xc (长度为1-20字符)
	// if (devName.replace(/[^\x00-\xff]/g, 'xx').length > 20) {
 ...
 * timeout：  num autoClose为true时多少秒后自动关闭
...
(function ($, window, undefined) {
	...
	function ModalDialog (opt) {
		...
		html.push('<h3 class="md-header"></h3>');
		html.push('<button class="md-close">×</button>');
	...
	ModalDialog.prototype = {
		...
		init: function () {
			...
			if (this.opt.autoClose) {
				window.setTimeout(function () {
					_this.hide();
					_this.opt.closeCallBack && _this.opt.closeCallBack();
		...
		//获取和设置弹出框尺寸
		getSize: function () {
			this.opt.MAX_Height = (window.innerHeight || (document.documentElement && document.documentElement.clientHeight) || document.body.clientHeight) * 0.9 - 40;
		...
		setButtons: function (btns) {
			...
			for (var i = 0, l = btns.length; i < l; i++) {
				(function (i) {
					var btn = btns[i],
						$btn = $('<button class="md-btn ' + btn.theme + '">' + btn.text + '</button>');
