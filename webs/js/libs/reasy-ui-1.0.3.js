    ...
    var Textboxs = {
        ...
        create: function (elem, type, defVal) {
            ...
            $(textboxs).on('focus', function () {
                ...
                this.parentNode.value = Textboxs.getValue(this.parentNode);
            });
            ...
            $elem.on('keydown', function (e) {
                ...
                if (elem.index !== (len - 1) && elem.value.length > 0) {
                    ...
                    if (elem.value.length === maxlength && pos === maxlength) {
                        ...
                        skipNext = true;
                    } else if (val[val.length - 1] == "." || val[val.length - 1] == "。") { //win10中文输入法不跳转输入框问题
                        if (val.length > 1) {
                            skipNext = true;
                        } else {
                            elem.value = replacedVal;
    ...
    var Inputs = {
        addCapTip: function (newField, pasElem, func) {
            ...
            $newField.on("keyup", function (e) {
                ...
                if (!this.capDetected && !func) {
                    ...
                    massageElm.id = msgId;
                    massageElm.innerHTML = _("You entered a capital letter.");
                    if (pasElem) {
        ...
        createTextInput: function (elem) {
            ...
            $newField.on("blur", function (e) {
                    ...
                    if (this.value !== "") {
                        $this.hide();
                        $elem.show();
                        $.validate.utils.check.call(elem, eventType);
                    } else {
                        $.validate.utils.check.call(this, eventType);
                    }
                }).on("focus", function (e) {
                    $elem.removeValidateTip(true);
        ...
        addPlaceholder: function (elem, placeholderText) {
            ...
            function createPlaceholderElem(elem) {
                //add by xc
                //去除可能重复的placeholder
                var $placeElem = $(elem).parent().children('.placeholder-content');
                if ($placeElem.length > 0 && placeholderText && $placeElem.text() == placeholderText) {
                    $placeElem.remove();
                }
    ...
    var Inputselect = {
        ...
        create: function (elem, obj) {
            ...
            $(elem).addClass('input-append');
                ...
                root = elem.getElementsByTagName('ul')[0];
                    ...
                    if (hasToggle) {
                        display =
                          ulList.style.display = (targetDis === 'none' ||
                            $.trim(targetDis) === '');
                        ulList.style.display = display ? 'block' : 'none';

                        if(display) { //显示时
                            ulListRect = ulList.getBoundingClientRect();
                            bodyWidth = document.body.scrollWidth || document.body.clientWidth;
                            if(ulListRect.right > bodyWidth) { //当超出body宽度时，下拉列表右对齐
                                ulList.style.left = target.getBoundingClientRect().right - ulListRect.right + "px";
                            }
                        }
...
(function (window, document) {
    ...
    $.validate = (function () {
        var handler = {
            ...
            keyup: function (e) {
                var data = utils.getOptions(this) || null,
                    eventType = e ? e.type : null;
                //去除数字前面的0
                if (data && typeof (data.type) !== "undefined") {
                    if (data.type === "num" && this.value != "") {
                        if (!$(this).parent().hasClass("has-error") && !isNaN(this.value)) {
                            this.value = Number(this.value);
                        }
                    }else if (data.type === "ipv6pool" && this.value != "") {
                    ...
                    } else if (data.type.indexOf("ipv6") != -1 && this.value != "") {
                        if (!$(this).parent().hasClass("has-error")) {
    ...
    valid = {
        ...
        mask: {
            all: function (str) {
                var rel = /^(255|254|252|248|240|224|192)\.0\.0\.0$|^(255\.(254|252|248|240|224|192|128|0)\.0\.0)$|^(255\.255\.(254|252|248|240|224|192|128|0)\.0)$|^(255\.255\.255\.(248|240|224|192|128|0))$/;
                if (!rel.test(str)) {
        ...
        wanmask: {
            all: function (str) {
                var rel = /^(255|254|252|248|240|224|192)\.0\.0\.0$|^(255\.(254|252|248|240|224|192|128|0)\.0\.0)$|^(255\.255\.(254|252|248|240|224|192|128|0)\.0)$|^(255\.255\.255\.(255|254|252|248|240|224|192|128|0))$/;
                if (!rel.test(str)) {
                    return _("Please enter a valid subnet mask.");
                }
        ...
        ipv6prefix: {
            all: function (str) {
                var items = str.split("::"),
                    ret = $.validate.valid.ipv6Header.all(str, true);
                if (ret) {
                    return _("请输入有效的前缀");
                }
                if (items.length != 2) { //only one ::
                    return (_("请输入有效的前缀"));
                }
                if (items[1] != "") { //::后面必须为空
                    return (_("请输入有效的前缀"));
        ...
        ipv6pool: {
            all: function (str) {
                var len = str.length,
                    reg = /^[1-9a-fA-F][0-9a-fA-F]{0,3}$/;
                if (!reg.test(str)) {
                    return _("Please enter a hexadecimal number, ranging from 1 to ffff");
                }
            }
        },

        //6to4 lanip 只能以2002开头
        ipv6_6to4: {
            all: function (str){
                var headStr = str.slice(0, 4),
                    ret = $.validate.valid.ipv6Value.all(str);
                if(headStr !== "2002"){
                    return (_("Please enter a valid address"));
                }
                if(ret){
                    return ret;
                }
        ...
        //全球单播地址
        ipv6Header: {
            all: function (str, isLanHeader) {
                var ret = this.specific(str, isLanHeader);
                if (ret) {
                    return ret;
                }
            },
            specific: function (str, isLanHeader) {
                var headStr = str.split("::")[0].split(":")[0],
                    headStr1,headStr2,headStr4,
                    errAddrHeader = ["2002"],
                    i = 0;
                ...
                if (headStr.length != 4) {
                    ...
                    headStr1 = str.slice(0, 1).toUpperCase();
                    headStr2 = str.slice(0, 2).toUpperCase(); //开头前2位
                    headStr4 = str.slice(0, 4).toUpperCase(); //开头前4位
                }

				//LAN 支持 FC和FD开头
                if (headStr1 != 2 && headStr1 != 3) {
					//LAN时 支持FD FC开头
					if(!isLanHeader || (headStr2 != "FD" && headStr2 != "FC")) {
						return (_("Please enter a valid address"));
					}
                }

                if(headStr2 == "FF"){  //组播地址从ff00开始
                    return (_("Please enter a valid address"));
                }

                for(i; i < errAddrHeader.length; i++){ //全球唯一的IPV6地址非法开头："2002"
                    if (headStr4 == errAddrHeader[i]) {
                        return (_("Please enter a valid address"));
                    }
                }

                if(Number(str.replace(/:/g,"")) == 0){ //不能为全0
                    return (_("Please enter a valid address"));
                }

                if(Number(str.replace(/:/g,"")) == 1 && str.slice(-1) == "1"){ //相当于ipv4的127.0.0.1
                    return (_("Please enter a valid address"));
                }
            }
        },
        ...
        ipv6Value: {
            all: function (str) {
                ...
                if (idx == -1) {
                    items = str.split(":");
                    if (items.length != 8) { //没有"::",必须有8节地址
                        return (_("Please enter a valid address"));
                    } else {
                        for (i = 0; i < items.length; i++) {
                            if (!(/^[0-9a-fA-F]{1,4}$/).test(items[i])) { //16进制
                                return (_("Please enter a valid address"));
                            }
                        }
                    }
                    ...
                    if (idx != str.lastIndexOf("::")) { //只能有一个"::"
                        return (_("Please enter a valid address"));
                    } else {
                        if (strLen > 8) {  //如果有双冒号，最多为7节地址
                            return (_("Please enter a valid address"));
                        }
                        items = str.split("::");
                        ...
                        items0.push.apply(items0,items1);
                        if (items[0] != "") {
                        ...
                        if (items[1] == "") {
                            ...
                            if(Number(items[1].replace(/:/g,"")) == 0){ //不能为全0
                                return (_("Please enter a valid address"));
                            }
                        }
                    }
        ...
        ipv6: {
            all: function (str) {
                var valueRet = $.validate.valid.ipv6Value.all(str),
                    headerRet = $.validate.valid.ipv6Header.all(str),
                    ret = headerRet || valueRet;
                if (ret) {
                    return ret;
                }
            }
        ...
        ipv6Mask: {
            all: function (str) {
                var rel = /^(255|254|252|248|240)\.0\.0\.0$|^(255\.(254|252|248|240|224|192|128|0)\.0\.0)$|^(255\.255\.(254|252|248|240|224|192|128|0)\.0)$|^(255\.255\.255\.(248|240|224|192|128|0))$/;
                if (!rel.test(str)) {
                    return _("Please enter a valid subnet mask.");
                }
        ...
        ipv6LocalLink: { //链路本地地址
            all: function (str) {
                var
                    // reg = /^FE(8|9|A|B)[0-9A-F]$/i,
                    //目前只支持FE80
                    ret,
                    header = str.slice(0,4).toUpperCase();
                    if(header != "FE80") {
                        ret = $.validate.valid.ipv6.all(str);
                    } else {
                        ret = $.validate.valid.ipv6Value.all(str);
                    }
        ...
        url: {
            all: function(str){
                ...
                if(!reg.test(str)){
                    return _("Please enter a corret URL");
                }
            }
...
$.extend($.validate.valid, {
    //fh1901 的一版用户名和密码的有效验证，英文字符，不能包含非法字符 ~ ; ' & “ 五个特殊字符
