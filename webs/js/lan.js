...
function changeIP() {
	if (G.data.lanIp != $("#lanIp").val()) {
			if (!confirm(_("The login IP address is changed to %s. You are required to relogin  with the new IP address, are you sure to continue?", [$("#lanIp").val()]))) {
				return false;
			}
		}
		return true;
