...
function PageView() {
	...
	this.setLockMsg = function(seconds) {
		...
		$('#login-message').html(_("已锁定，请%s秒后重试！", [seconds]));
	...
	this.showSuccessful = function (str) {
		var num = str;
		if (num == 1) { //密码错误
			$('#login-message').html(_("用户名或密码错误!"));
		} else if (num == 2) { //用户名错误
			$('#login-message').html("用户名或密码错误!");
		} else if(num == 3) { //已有账户登录
			$('#login-message').html("已有账户登录!");
		} else if(num == 4) { //锁定
			_this.getData();
		} else {
...
$(function () {
	...
	$('#login-password').initPassword(_("Password"), true, false, function () {
