...
function checkData() {
	G.validate = $.validate({
		custom: function () {
            ...
            //是否与ftp端口重复
            for (; i < usbport.length; i++) {
                if (outPort == usbport[i]) {
                    outPortRepeat = 2;
                    break;
                }
            }
			...
			} else if (outPortRepeat == 2) {
                return _("The extranet port cannot be duplicate with FTP port");
            }
