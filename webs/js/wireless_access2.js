...
var pageModel = R.pageModel({
	getUrl: "goform/getWifiFilterCfg",
	setUrl: "goform/setWifiFilterCfg",
...
var moduleModel = R.moduleModel({
	...
	getSubmitData: function () {
		...
		listStr = "";
		for (i = 0; i < len; i++) {
			if (trArr.eq(i).children().length != 2) {
				continue;
			}
			listStr += "" + "\r" + trArr.eq(i).children().eq(0).text() + "\n";
		}
...
function addTableList(tableId) {
	var	mac = $(this).parents("tr").find(".mac").val().toUpperCase(),
	...
	if (len >= MAX_LENGTH) {
		showErrMsg("msg-err", _("Only a maximum of %s rules are allowed.", [MAX_LENGTH]));
		return;
	}
	for (i = 0; i < len; i++) {
		if (mac == $tr.eq(i).children().eq(0).text()) {
...
function addOnlineList() {
	...
	for (i = 0; i < len; i++) {
		...
		$("#whiteListTable tr:gt(1)").each(function () {
			//判断MAC地址是否存在
			if ($(this).children().eq(0).html().toLowerCase() == onlineArr[i].devMac.toLowerCase()) {
				hasInTable = true;
				return false;
			}
		});
...
function createTableList(dataArr, tableId, type) {
	...
	if (len === 0 && type) {
		//黑名单为空时
		if (type == "black") {
			trStr = "<tr><td class='black-link' colspan='2'>" + _("The blacklist is empty.") + "</td>";
		} else {
			if (initWhiteLength === 0) {
				...
				trStr = "<tr><td colspan='2' style='text-align: center;'><span class='online-link'>" + _("Add all online devices to the whitelist") + "</span></td></tr>";
			}
		}
	...
	for (i = 0; i < len; i++) {
		...
		trStr = "<tr><td title='" + deviceMac + "'>" + deviceMac + "</td>";
		...
		//$("#" + tableId + " tbody .device-target").html(toHtmlCode(deviceName));
