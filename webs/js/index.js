...
function clickElink () {
    if ($(this).hasClass("btn-on")) {
        ...
        $(this).val(0);
        $("#wrlPwd").removeClass("none");
        $("#ssid").removeAttr("disabled");
    } else {
        $(this).attr("class", "btn-on");
        $(this).val(1);
        $("#wrlPwd").addClass("none");
        $("#ssid").attr("disabled", "true");
        $("#ssid").val(G.data.upperSsid || G.data.ssid );
    }
...
function initValue (obj) {
    ...
    // eLink开关打开
    if(G.data.elinkEn == 1) {
        $("#eLink").attr("class", "btn-on").val(1);
        $("#wrlPwd").addClass("none");
        $("#ssid").attr("disabled", "true");
        $("#ssid").val(G.data.upperSsid || G.data.ssid );
    } else {// 关闭
        $("#eLink").attr("class", "btn-off").val(0);
    }
...
function nextSet () {
    ...
    /*else if (!$("#dhcp_setting").hasClass("none")) {
        ...
        if (((wrlPwd !== "") && ($("#hideWrlPwd")[0].checked === false)) || $("#eLink").val() == "1") {

            if($("#eLink").val() != "1") { // elink未开启
            ...
            var subObj = {
                "ssid": $("#ssid").val(),
                "wrlPassword": ($("#hideWrlPwd").prop("checked") || $("#eLink").val() == "1") ? "" : wrlPwd,
                "power": $("#power").val(),
...
function showFinish (ssid_5g) {
    ...
    if (G.wanStatus == "7") {
        ...
        $("#ssid_5g").html(toHtmlCode(ssid_5g));
...
function handWifi (data) {
    var index = 0;
    var objData = $.parseJSON(data);
    if (G.wanStatus == "7") {
        showFinish(objData.ssid_5g);
        return;
    }
    $("#waiting").removeClass("none");
    $("#wifi_setting").addClass("none");
